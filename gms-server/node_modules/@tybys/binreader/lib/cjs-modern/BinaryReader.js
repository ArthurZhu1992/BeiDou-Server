...
const bufferMethods = require("./buffer.js");
const Reader_1 = require("./Reader.js");
const node_1 = require("./node.js");
const EndianType_1 = require("./EndianType.js");
const FileDescriptor_1 = require("./FileDescriptor.js");
...
function readNumber(reader, method) {
    ...
    if (reader.type === 0 /* FILE */) {
        readLength = node_1.fs.readSync(reader._file, buf, 0, methods[method], reader.pos);
    }
    else {
        readLength = bufferMethods.copy(reader._buffer, buf, 0, reader.pos, reader.pos + methods[method]);
    }
 ...
 * @public
 */
class BinaryReader extends Reader_1.Reader {
    constructor(buffer, endian = EndianType_1.EndianType.BigEndian) {
        if (typeof buffer === 'string') {
            super(node_1.fs.statSync(buffer).size);
            Object.defineProperty(this, 'type', { configurable: true, enumerable: true, writable: false, value: 0 /* FILE */ });
            this._file = node_1.fs.openSync(buffer, 'r');
            this._path = buffer;
            this._buffer = null;
        }
        ...
        else if (buffer instanceof FileDescriptor_1.FileDescriptor) {
            super(buffer.size);
            Object.defineProperty(this, 'type', { configurable: true, enumerable: true, writable: false, value: 0 /* FILE */ });
    ...
    read(len = 1) {
        ...
        let readLength;
        if (this.type === 0 /* FILE */) {
            readLength = node_1.fs.readSync(this._file, buf, 0, len, this.pos);
        }
        else {
    ...
    readToBuffer(buf, bufStart = 0, len = 1) {
        ...
        let readLength;
        if (this.type === 0 /* FILE */) {
            readLength = node_1.fs.readSync(this._file, buf, bufStart, len, this.pos);
        }
        else {
    ...
    readString(encoding = 'ascii', length = -1) {
        ...
        if (length === -1) {
            ...
            do {
                let readLength;
                if (this.type === 0 /* FILE */) {
                    readLength = node_1.fs.readSync(this._file, buf, 0, 1, this.pos + l);
                }
                else {
            ...
            if (this.type === 0 /* FILE */) {
                node_1.fs.readSync(this._file, r, 0, l - 1, this.pos);
            }
            else {
