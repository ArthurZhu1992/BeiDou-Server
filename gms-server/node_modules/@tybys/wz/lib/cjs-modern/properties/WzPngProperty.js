...
const Color_1 = require("../util/Color.js");
...
const Canvas_1 = require("../util/Canvas.js");
let wasmInflate = null;
/**
 * @public
 */
class WzPngProperty extends WzExtended_1.WzExtended {
    ...
    get propertyType() {
        return WzPropertyType_1.WzPropertyType.PNG;
    }
    dispose() {
        if (this._disposed)
            return;
    ...
    setValue(_value) {
        throw new NotImplementedError_1.NotImplementedError('[WzPngProperty#setValue]');
    }
    ...
    async parsePng(saveInMemory = false) {
        ...
        switch (format) {
            case 1: {
                ...
                const img = new Canvas_1.Canvas(this.width, this.height);
                // img.colorType(6)
            ...
            default: {
                ErrorLogger_1.ErrorLogger.log(ErrorLogger_1.ErrorLevel.MissingFeature, `Unknown PNG format ${this.format1} ${this.format2}`);
                break;
            }
        }
        this.png = bmp;
    ...
    async _getRawImage(saveInMemory = false) {
        ...
        const reader = new binreader_1.BinaryReader(rawImageBytes);
    ...
    static getPixelDataDXT5(rawData, width, height) {
        ...
        for (let y = 0; y < height; y += 4) {
            for (let x = 0; x < width; x += 4) {
                ...
                const r = new binreader_1.BinaryReader(rawData);
                r.seek(off + 8);
    ...
    static expandColorTable(color, c0, c1) {
        ...
        if (c0 > c1) {
            color[2] = Color_1.Color.fromArgb(0xff, parseInt((color[0].r * 2 + color[1].r + 1) / 3), parseInt((color[0].g * 2 + color[1].g + 1) / 3), parseInt((color[0].b * 2 + color[1].b + 1) / 3));
            color[3] = Color_1.Color.fromArgb(0xff, parseInt((color[0].r + color[1].r * 2 + 1) / 3), parseInt((color[0].g + color[1].g * 2 + 1) / 3), parseInt((color[0].b + color[1].b * 2 + 1) / 3));
        }
        else {
            color[2] = Color_1.Color.fromArgb(0xff, parseInt((color[0].r + color[1].r) / 2), parseInt((color[0].g + color[1].g) / 2), parseInt((color[0].b + color[1].b) / 2));
            color[3] = Color_1.Color.fromAc(0xff, Color_1.Color.black);
        }
    }
...
exports.WzPngProperty = WzPngProperty;
function inflate(data, len) {
    if (typeof window !== 'undefined') {
        return inflateWasm(data, len);
    }
    return new Promise((resolve, reject) => {
        const inflateStream = node_1.zlib.createInflate();
        const buf = node_1._Buffer.alloc(len);
        const chunks = [];
        ...
        inflateStream.on('finish', () => {
            const chunk = node_1._Buffer.concat(chunks);
...
async function inflateWasm(data, len) {
    if (wasmInflate === null) {
        const wzWasm = await Promise.resolve().then(() => require("../util/wz.js"));
...
function rgb565ToColor(val) {
    ...
    const c = Color_1.Color.fromRgb((r << 3) | (r >> 2), (g << 2) | (g >> 4), (b << 3) | (b >> 2));
    return c;
}
function bgra8888(img, data, length) {
    ...
    for (let i = 0; i < length; i += 4) {
        img.setPixelColor(Canvas_1.Canvas.rgbaToInt(data[i + 2], data[i + 1], data[i + 0], data[i + 3]), x, y);
        x++;
        if (x >= width) {
            x = 0;
            y++;
...
function rgb565(img, data, length) {
    ...
    let y = 0;
    const width = img.getWidth();
    const r = new binreader_1.BinaryReader(data);
    for (let i = 0; i < length; i += 2) {
        const ushort = r.readUInt16LE(); // data.readUInt16LE(i)
        const c = rgb565ToColor(ushort);
        img.setPixelColor(Canvas_1.Canvas.rgbaToInt(c.r, c.g, c.b, c.a), x, y);
        x++;
        if (x >= width) {
            x = 0;
            y++;
...
function argb1555ToColor(val) {
    ...
    const g = (val & maskG) >> 5;
    const b = (val & maskB);
    const c = Color_1.Color.fromArgb(a, (r << 3) | (r >> 2), (g << 3) | (g >> 2), (b << 3) | (b >> 2));
    return c;
}
//# sourceMappingURL=WzPngProperty.js.map